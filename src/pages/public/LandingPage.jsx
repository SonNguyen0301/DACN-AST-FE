...
export default function LandingPage() {
  ...
  const navItems = [
    { label: 'Trang chủ', ref: homeRef },
    { label: 'Tính năng', ref: featureRef },
    { label: 'Quy trình', ref: processRef },
    { label: 'Công nghệ', ref: techRef },
    { label: 'Đối tượng', ref: audienceRef }
  ];
  ...
  return (
    <Layout style={{ minHeight: '100vh', background: '#fff', fontFamily: "'Inter', sans-serif" }}>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }} onClick={() => scrollToSection(homeRef)}>
          <img 
            src="/ASTCare1.png" 
            alt="ATSCare Logo" 
            width="140" 
      ...
      <Content>
        {/* SECTION: TRANG CHỦ */}
        <div ref={homeRef} className="hero-padding" style={{ background: 'linear-gradient(180deg, #fff 0%, #f8fbff 100%)' }}>
          <Row gutter={[64, 48]} align="middle">
            <Col xs={24} md={12}>
              ...
              <Title className="hero-title" style={{ fontWeight: 800, lineHeight: 1.2, marginBottom: 24, color: '#1f2937' }}>
              ...
              <Space size="middle">
                <Button type="primary" style={btnPrimaryStyle} onClick={() => navigate('/patient/booking')}>
            ...
            <Col xs={24} md={12} style={{ textAlign: 'center', position: 'relative', padding: '20px' }}>
              ...
              <div style={{ 
                ...
                width: '90%', height: '90%', background: 'radial-gradient(circle, rgba(22,119,255,0.2) 0%, rgba(255,255,255,0) 70%)', 
              ...
              <div style={{
                  position: 'absolute', bottom: '10%', left: '-5%', background: '#fff', padding: '15px 25px',
                  borderRadius: '20px', boxShadow: '0 15px 30px rgba(0,0,0,0.1)', display: 'flex',
                  alignItems: 'center', gap: '15px', zIndex: 2, animation: 'bounce 3s infinite ease-in-out' 
              }}>
        ...
        {/* SECTION: TÍNH NĂNG */}
        <div ref={featureRef} className="section-padding" style={{ background: '#fff' }}>
                ...
                <Carousel ref={carouselRef} autoplay autoplaySpeed={3000} effect="fade" dots={false}>
                  {featureSlides.map((slide) => (
                    <div key={slide.key} style={{ position: 'relative', padding: '10px' }}>
                      ...
                      />
                      <div style={{ 
                        position: 'absolute', bottom: 40, left: -10, background: '#fff', padding: '12px 20px', 
                        borderRadius: 12, boxShadow: '0 10px 25px rgba(0,0,0,0.1)', display: 'flex', 
                        alignItems: 'center', gap: 10, border: '1px solid #f0f0f0', zIndex: 10
                      }}>
        ...
        {/* SECTION: QUY TRÌNH */}
        <div ref={processRef} className="section-padding" style={{ background: '#f9fafb' }}>
          ...
          <Row gutter={[32, 32]}>
            {[
              { step: '01', title: 'Tải ảnh lên', icon: <CloudUploadOutlined />, desc: 'Chụp ảnh vùng da tổn thương và mô tả triệu chứng.', color: '#1677ff', bg: '#e6f4ff' },
              { step: '02', title: 'Phân tích AI', icon: <ScanOutlined />, desc: 'AI xử lý hình ảnh, so sánh với cơ sở dữ liệu y khoa.', color: '#fa8c16', bg: '#fff7e6' },
              { step: '03', title: 'Nhận kết quả', icon: <FileProtectOutlined />, desc: 'Nhận gợi ý chẩn đoán sơ bộ và mức độ nghiêm trọng.', color: '#52c41a', bg: '#f6ffed' },
              { step: '04', title: 'Kết nối Bác sĩ', icon: <ScheduleOutlined />, desc: 'Đặt lịch khám với bác sĩ hoặc nhập viện nếu cần.', color: '#722ed1', bg: '#f9f0ff' },
            ].map((item, index) => (
              <Col xs={24} md={6} key={index}>
                ...
                >
                  <div style={{ 
                    width: 70, height: 70, background: item.bg, borderRadius: '50%', margin: '0 auto 20px', 
                    display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 30, color: item.color 
                  }}>
                    {item.icon}
                  </div>
        ...
        {/* SECTION: CÔNG NGHỆ */}
        <div ref={techRef} className="section-padding" style={{ background: '#001529', color: '#fff' }}>
          ...
          <Row gutter={[24, 24]}>
            {[
              ...
              <Col xs={24} md={12} key={index}>
                <div 
                  style={{ 
                    padding: '32px', background: 'rgba(255,255,255,0.05)', borderRadius: 24, 
                    border: '1px solid rgba(255,255,255,0.1)', height: '100%', transition: 'all 0.3s', cursor: 'default'
                  }}
                ...
                >
                  <div style={{ display: 'flex', gap: 20, alignItems: 'flex-start' }}>
                    <div style={{ 
                      padding: 16, background: 'rgba(255,255,255,0.05)', borderRadius: 16, 
        ...
        {/* SECTION: ĐỐI TƯỢNG */}
        <div ref={audienceRef} className="section-padding" style={{ background: '#fff' }}>
          <div style={{ textAlign: 'center', marginBottom: 60 }}>
            <Title level={2} style={{ fontSize: 36, fontWeight: 800 }}>Ai nên sử dụng ASTCare?</Title>
          </div>
          <Row gutter={[32, 32]}>
            {[
              { role: 'Bệnh nhân', desc: 'Người có nhu cầu kiểm tra da, lo lắng về triệu chứng lạ, muốn đặt lịch nhanh.', icon: <UserOutlined />, color: '#1677ff', bg: '#e6f4ff' },
              { role: 'Bác sĩ Da liễu', desc: 'Cần công cụ hỗ trợ chẩn đoán nhanh, quản lý bệnh nhân hiệu quả hơn.', icon: <MedicineBoxOutlined />, color: '#722ed1', bg: '#f9f0ff' },
              { role: 'Bệnh viện', desc: 'Cần tối ưu quy trình tiếp nhận, giảm tải áp lực trong giờ cao điểm.', icon: <BankOutlined />, color: '#52c41a', bg: '#f6ffed' },
            ].map((item, idx) => (
              <Col xs={24} md={8} key={idx}>
                <Card hoverable bordered={false} style={{ borderRadius: 24, textAlign: 'center', padding: 20, border: '1px solid #f0f0f0', height: '100%' }}>
